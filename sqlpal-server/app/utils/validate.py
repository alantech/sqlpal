...
def _is_valid_where_clause(clause, columns):
    ...
    if clause is not None and hasattr(clause, 'lexpr') and isinstance(clause.lexpr, ast.ColumnRef) and list(clause.lexpr.fields).pop().sval not in columns:
        return False
    elif clause is not None and hasattr(clause, 'rexpr') and isinstance(clause.rexpr, ast.ColumnRef) and list(clause.rexpr.fields).pop().sval not in columns:
        return False
    return True
