...
export async function autocomplete(
  ...
  // );
  // return resp.json();
  return await window.electron.server.autocomplete(backendUrl, connString, sql, dialect);
}
...
export async function discoverData(backendUrl: string, connString: string, dialect: string, schema: Schema) {
  ...
  // return {};
  // todo: fix cast
  return await window.electron.server.discover(backendUrl, connString, dialect, schema as any);
}
...
export async function repair(
  ...
  // );
  // return resp.json();
  return await window.electron.server.repair(backendUrl, connString, query, error, dialect);
}
...
export async function addStatement(backendUrl: string, connString: string, query: string, dialect: string) {
  ...
  // return resp.json();
  return await window.electron.server.add(backendUrl, connString, query, dialect);
}
