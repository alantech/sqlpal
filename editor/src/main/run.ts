...
async function runSql(
  ...
  const stmts = Object.values(parsedSql?.parsedQueries ?? {}).map(
    (q: ParsedQuery) => q.query
  );
      ...
      return out.map((t: { statement: any; queryRes: QueryResult }) => {
        if (
          ...
          return {
            ...
            affected_records: t.queryRes.rowCount,
          };
          ...
          return {
            ...
            types: Object.fromEntries(
              t.queryRes.fields.map((f) => [f.name, f.dataTypeID])
            ),
          };
        } else {
          return {
            statement: t.statement,
            error: `unexpected result: ${t.queryRes}`,
          }; // TODO: Error this out
        }
      });
    }
    case KnexClient.MYSQL: {
      return out.map((t: { statement: any; queryRes: any }) => {
        ...
        if (
          ...
          return {
            statement: t.statement,
            error: `unexpected result: ${JSON.stringify(t.queryRes)}`,
          }; // TODO: Error this out
        }
      });
    }
